...
class AgentManager:
    ...
    def __init__(self, loader, clock):
        ...
        # ── 同步時鐘日期至角色資料的最小天數 ─────────────────────
        if self._characters:
            min_day = min(c.day for c in self._characters.values())
            self.clock.set_day(min_day)
            logger.info(f"時鐘同步至第 {min_day} 天")
    ...
    def run_tick(self,
              ...
              external_input: dict = None,
              image_input: dict = None) -> dict:
        """
        執行一個完整 tick。
        ...
        # 確保所有 perception 都含有時間資訊（供 STM 記錄用）
        for code in list(perception_input.keys()):
            perception_input[code].setdefault("time_str",     self.clock.time_str)
            perception_input[code].setdefault("time_minutes", self.clock.time_minutes)
        ...
        decide_results = self._decide_phase(
            perception_input,
            external_input,
            interrupt_results,
            image_input,
    ...
    def _execute_phase(self, perception_input: dict) -> dict:
        ...
        for code in sorted(self._agents.keys()):
            ...
            if not action:
                # 從時間表取（限時間已到達的時段，避免超前執行）
                slot = char.get_current_slot(self.clock.time_minutes)
                if slot:
                    action = slot["action"]
            ...
            elif action == "回家" and target:
                # 回家同樣更新位置（location 欄位即住所）
                char.current_location = target
            char.add_today_action(action)

            # 標記時間表格已完成（當執行行動符合目前排班時段時）
            _sched = char.get_current_slot(self.clock.time_minutes)
            if _sched:
                sched_action = _sched.get("action", "")
                sched_loc    = _sched.get("location", "")
                # 精確匹配：行動名稱相同
                exact_match = (sched_action == action)
                # 語意等效：「前往 <住所>」視同「回家」（Markov 有時選前往而非回家）
                home_equiv  = (sched_action == "回家" and
                               action == "前往" and
                               target == sched_loc)
                if exact_match or home_equiv:
                    char.mark_slot_completed(_sched["time"])
    ...
    def _decide_phase(self, perception_input, external_input,
                  interrupt_results=None, image_input=None) -> dict:
        """所有角色決策下一 tick 行動。"""
        interrupt_results = interrupt_results or {}
        image_input = image_input or {}
        results = {}
        ...
        for code in sorted(self._agents.keys()):
            ...
            # ── default_sleep_time 到達 → 直接入睡，不走 agent.decide() ──
            # 這確保角色在排程的睡覺時間確實入睡，而非被 Markov 慣性拖延。
            # force_sleep_time 是最後底線；default_sleep_time 是計劃目標。
            if self._is_sleep_time_for(char):
                _perc = perception_input.get(code, {})
                _stm  = self._stms[code]
                _turn_num = _stm.next_turn_number(char.day)
                _turn_id  = make_turn_id(char.day, _turn_num)
                _stm.add_turn(
                    turn_id    = _turn_id,
                    time       = self.clock.time_str,
                    perception = _perc,
                    event      = {"input_text": "", "action": SLEEP_ACTION,
                                  "target": "", "content": ""},
                    inner      = {"thought": f"{char.name}感到疲憊，決定入睡。",
                                  "emotion": char.emotion},
                    tick_id    = _turn_id,
                )
                logger.info(f"[{code}] 到達 default_sleep_time，安排入睡")
                self._do_sleep(code)
                results[code] = {
                    "action": SLEEP_ACTION, "target": "", "mode": "schedule",
                    "should_sleep": True, "_meta": {},
                }
                continue
            ...
            # ── 組裝 Salience-First 參數 ─────────────────────────────
            perception = perception_input.get(code, {})
            curr_yolo  = perception.get("yolo_desc", [])
            if isinstance(curr_yolo, str):
                curr_yolo = [curr_yolo] if curr_yolo else []

            delta_people = {
                "arrived":  [p for p in curr_yolo  if p and p not in char.prev_people],
                "departed": [p for p in char.prev_people if p not in curr_yolo],
            }

            # this_tick_event：優先用中斷事件，其次用 STM 最新筆（若本 tick 有寫入）
            interrupt_res = interrupt_results.get(code, {})
            if interrupt_res.get("interrupted") and interrupt_res.get("event"):
                evt_data = interrupt_res["event"].get("data", {})
                this_tick_event = (
                    evt_data.get("text", "")
                    or evt_data.get("description", "")
                    or evt_data.get("content", "")
                    or ""
                )
            else:
                current_tick_id = f"D{char.day:03d}_T{self._stms[code].count():03d}"
                this_tick_event = self._stms[code].get_latest_if_this_tick(current_tick_id)
            ...
            try:
                result = agent.decide(
                    ...
                    image            = image_input.get(code),

                    this_tick_event  = this_tick_event,
                    delta_people     = delta_people,
                    curr_people      = [p for p in curr_yolo if p and p != ""],
                )

                # 更新 prev_people（供下一 tick 計算 delta_people）
                char.update_prev_people([p for p in curr_yolo if p and p != ""])
                ...
                if result.get("action") == SLEEP_ACTION:
                    if not self._is_sleep_time_for(char):
                        ...
                        char.set_pending_action("休息", "")
                        # STM 最後一筆已記為「睡覺」→ 修正為「休息」
                        # （避免 Markov 慣性被虛假睡覺紀錄污染）
                        _stm_turns = self._stms[code]._stm.get("turns", [])
                        if _stm_turns and _stm_turns[-1].get("event", {}).get("action") == SLEEP_ACTION:
                            _stm_turns[-1]["event"]["action"] = "休息"
